  ...
  return {
    ...
    handleValueChange: (e: any) => {
      ...
      if (!refIsComposition.current) {
        ...
        if (compositionValue) setCompositionValue(undefined);
        triggerValueChange(newValue, e);
      } else {
        setCompositionValue(newValue);
      }
    },
    // intercept handleKeyDown without affecting the default behavior during input chinese
    handleKeyDown: (e: any) => {
      const keyCode = e.key;
      if (!refIsComposition.current) {
        onKeyDown?.(e);
        if (keyCode === 'Enter') {
          ...
          if (normalizeHandler) {
            const normalized = normalizeHandler('onPressEnter')(e.target.value);
            triggerValueChange(normalized, e);
          }
        }
      }
    },
