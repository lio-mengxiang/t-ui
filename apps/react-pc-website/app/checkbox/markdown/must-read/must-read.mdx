...
<H1 subheading="高拓展性 checkbox 实现思路">拓展性如此强大的 Checkbox，有必要了解一下！</H1>

<ImageTypography src="/checkbox/checkbox.webp" fill alt="button" />
...
<ImageTypography src="/checkbox/default-checkbox.webp" fill alt="popover" imageClassName="object-contain" />
...
<CodeBlock
  code={`if (context.isCheckboxGroup) {
  onGroupChange?.(mergeProps.value, e.target.checked, e);
} else {
  setChecked(e.target.checked); =
}`}
...
<CodeBlock
  code={`<CheckboxContext.Provider value={{ checked, disabled, readonly, indeterminate }}>
...
<CodeBlock
  code={`<CheckboxContext.Provider value={{ checked, disabled, readonly, indeterminate }}>
      <label {...rest} onClick={onLabelClick} aria-disabled={!!disabled} aria-readonly={!!readonly} aria-checked={!!checked}>
        {/* 为什么没有 readonly 状态， 标准里本来也没有 readonly 属性 */}
        <input
          ref={inputRef}
          disabled={!!disabled}
          value={value}
...
<CodeBlock
  code={`import { Checkbox } from '@t-headless-ui/react';
// use checkbox
<Checkbox.Root>
    Checkbox
</Checkbox.Root>
...
<Checkbox.Group defaultValue={['1']}>
    <Checkbox.Root value="1">Option 1</Checkbox.Root>
    <Checkbox.Root value="2">Option 2</Checkbox.Root>
</Checkbox.Group>
`}
  language="tsx"
/>
