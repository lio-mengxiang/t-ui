...
import { PAGINATION_PATH, menuLinks } from '@/constants';
...
const data = (router: LocaleRouter, size: string) => [
  {
    ...
    items: [
      ...menuLinks
        .filter((item) => item.path !== PAGINATION_PATH)
        .map((item) => ({
  ...
  },
  {
    ...
    onClick: () => {
      router.push(PAGINATION_PATH);
    },
    value: PAGINATION_PATH,
  },
  {
    content: (
      <MenuItem>
    ...
    onClick: () => {
      router.push(`${PAGINATION_PATH}/interview`);
    },
    value: `${PAGINATION_PATH}/interview`,
  },
  // {
  //   content: (
  //     <SubMenu>
  ...
  // },
  {
    content: (
      <SubMenu>
        <IconClickFill size="1.6em" className="text-[#4cc3fa]" /> Pagination 完整案例
      </SubMenu>
    ),
    value: `${PAGINATION_PATH}/example`,
    items: [
      {
        content: <MenuItem className="text-sm font-light">传统案例</MenuItem>,
        onClick: () => {
          router.push(`${PAGINATION_PATH}/example/tailwind-traditional-example`);
        },
        value: `${PAGINATION_PATH}/example/tailwind-traditional-example`,
      },
    ],
  },
];
