...
import { INPUT_PATH, menuLinks } from '@/constants';
...
const data = (router: LocaleRouter, size: string) => [
  {
    ...
    items: [
      ...menuLinks
        .filter((item) => item.path !== INPUT_PATH)
        .map((item) => ({
    ...
    items: [
      {
        ...
        onClick: () => {
          router.push(`${INPUT_PATH}/custom-css`);
        },
        value: `${INPUT_PATH}/custom-css`,
      },
      {
        content: <MenuItem className="text-sm font-light">Tailwind Input</MenuItem>,
        onClick: () => {
          router.push(`${INPUT_PATH}/tailwind-css`);
        },
        value: `${INPUT_PATH}/tailwind-css`,
      },
    ],
  },
  {
    ...
    value: `${INPUT_PATH}/example`,
    items: [
      {
        content: <MenuItem className="text-sm font-light">传统案例</MenuItem>,
        onClick: () => {
          router.push(`${INPUT_PATH}/example/tailwind-traditional-example`);
        },
        value: `${INPUT_PATH}/example/tailwind-traditional-example`,
      },

      {
        content: <MenuItem className="text-sm font-light">创意案例</MenuItem>,
        onClick: () => {
          router.push(`${INPUT_PATH}/example/tailwind-creative-example`);
        },
        value: `${INPUT_PATH}/example/tailwind-creative-example`,
      },
    ],
  },
];
