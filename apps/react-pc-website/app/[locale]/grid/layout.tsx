...
import { GRID_PATH, menuLinks } from '@/constants';
...
const data = (router: LocaleRouter, size: string) => [
  {
    ...
    items: [
      ...menuLinks
        .filter((item) => item.path !== GRID_PATH)
        .map((item) => ({
  ...
  },
  {
    ...
    value: GRID_PATH + '/example',
    items: [
      {
        content: <MenuItem className="text-sm font-light">传统案例</MenuItem>,
        onClick: () => {
          router.push(GRID_PATH + '/example/tailwind-traditional-example');
        },
        value: GRID_PATH + '/example/tailwind-traditional-example',
      },

      {
        content: <MenuItem className="text-sm font-light">创意案例</MenuItem>,
        onClick: () => {
          router.push(GRID_PATH + '/example/tailwind-creative-example');
        },
        value: GRID_PATH + '/example/tailwind-creative-example',
      },
    ],
  },
];
